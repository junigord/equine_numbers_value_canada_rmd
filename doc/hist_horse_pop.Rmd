...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Historical number of horses per province across Canada + out.width = '100%'"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caotion = "Max Horse Table")
```

Below we zoom in and look at the province of \`r params$province\`:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Case example: Historical number of horses" + out.width = '100%'}
